...
export default function Donations() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      .catch(err => console.error("Error fetching user profile for donation prefill:", err));
    }
  }, []);
  ...
  const handleDonate = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    if (!token) {
      alert("You must login or register a Samaj profile first to make a donation.");
      router.push("/login?redirect=donations");
      return;
    }

    try {
      const res = await fetch(getApiUrl("/api/v1/donations"), {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        setReceiptNum(`AS-DON-${data.payment?.id || Math.floor(100000 + Math.random() * 900000)}`);
        setIsSuccess(true);
      } else {
        const errorData = await res.json();
        alert(errorData.detail || "Failed to process donation. Please try again.");
      }
    } catch (err) {
      console.error("Donation error:", err);
      alert("Error contacting donation server.");
    }
  };

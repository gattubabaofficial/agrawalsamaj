...
async def create_booking(
    ...
    # Eagerly load facility and user relations to prevent lazy loading errors in serialization
    result = await db.execute(
        select(Booking)
        .options(selectinload(Booking.facility), selectinload(Booking.user))
...
async def create_donation(
    ...
    donation = Donation(
        donor_samaj_id=current_user.samaj_id,
    ...
    # Eagerly load user and payment relations to prevent lazy loading errors in serialization
    result = await db.execute(
        select(Donation)
        .options(selectinload(Donation.user), selectinload(Donation.payment))
...
@app.get(f"{settings.API_V1_STR}/bookings/my-bookings", response_model=List[BookingResponse])
...
@app.put(f"{settings.API_V1_STR}/bookings/{{booking_id}}/approve", response_model=BookingResponse)
async def approve_booking(
    ...
    # Reload booking with selectinload to ensure serialization doesn't crash
    result = await db.execute(
        select(Booking)
        .options(selectinload(Booking.facility), selectinload(Booking.user))
        .filter(Booking.id == booking_id)
...
@app.put(f"{settings.API_V1_STR}/bookings/{{booking_id}}/reject", response_model=BookingResponse)
...
@app.get(f"{settings.API_V1_STR}/donations", response_model=List[DonationResponse])
...
# ==============================================================================
# EVENTS UPDATE / DELETE (ADMIN)
# ==============================================================================

@app.put(f"{settings.API_V1_STR}/events/{{event_id}}", response_model=EventResponse)
async def update_event(
    ...
    event.fee_amount = payload.fee_amount
    
    await db.commit()
    await db.refresh(event)
    return event

@app.delete(f"{settings.API_V1_STR}/events/{{event_id}}")

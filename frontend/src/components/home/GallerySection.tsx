...
export default function GallerySection() {
  ...
  return (
    <section className="py-24 bg-zinc-50/50 dark:bg-zinc-950/20 border-t border-zinc-200/50 dark:border-zinc-800/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        {/* Filters */}
        <div className="flex gap-2 items-center justify-center border-b border-zinc-200/50 dark:border-zinc-800/50 pb-6">
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5 transition-colors ${
              filter === "all"
                ...
                : "bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-850"
            }`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredMedia.map((media, idx) => (
            <motion.div
              ...
              className="group overflow-hidden rounded-3xl border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm flex flex-col hover:shadow-md hover:border-amber-500/20 dark:hover:border-amber-500/20 transition-all duration-300"
            >
              {/* Media placeholder */}
              <div className="relative aspect-video w-full bg-zinc-100 dark:bg-zinc-800/50 flex flex-col items-center justify-center border-b border-zinc-100 dark:border-zinc-800/50 overflow-hidden">
                {media.type === "video" ? (
                  <Video className="w-12 h-12 text-zinc-400 dark:text-zinc-600 group-hover:scale-110 transition-transform duration-300" />
                ) : (
                  <ImageIcon className="w-12 h-12 text-zinc-400 dark:text-zinc-600 group-hover:scale-110 transition-transform duration-300" />
                )}
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-2">
                <h3 className="font-bold text-sm text-zinc-900 dark:text-white leading-snug line-clamp-2 group-hover:text-amber-500 transition-colors">
                  {media.title}
                </h3>

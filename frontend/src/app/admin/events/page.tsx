...
export default function AdminEventsPage() {
  ...
  const [activeView, setActiveView] = useState<"events" | "form" | "registrations">("events");
  ...
  const handleOpenCreate = () => {
    setFormData({
      ...
      is_members_only: false, timeline: []
    });
    setEditingId(null);
    setActiveView("form");
  };
  ...
  const handleOpenEdit = (evt: any) => {
    setFormData({
      ...
      is_members_only: evt.is_members_only || false,
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="number" min="0" value={formData.pass_price} onChange={e => setFormData({...formData, pass_price: Number(e.target.value)})} className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 focus:outline-none focus:border-amber-500 text-sm" placeholder="0 for Free" />
                <p className="text-xs text-zinc-500">Set to 0 to make it a Free Event.</p>
              </div>
              <div className="space-y-2">
            ...
            <div className="flex items-center gap-3 bg-rose-50 p-4 rounded-xl border border-rose-100">
              <input type="checkbox" id="membersOnly" checked={formData.is_members_only} onChange={e => setFormData({...formData, is_members_only: e.target.checked})} className="w-5 h-5 text-rose-600 border-rose-300 rounded focus:ring-rose-500" />
              <label htmlFor="membersOnly" className="text-sm font-semibold text-rose-900 cursor-pointer flex items-center gap-2">
                <ShieldAlert className="w-4 h-4" /> Members Only Event
                <span className="text-xs font-normal text-rose-600 block sm:inline">(Only registered Samaj members can join)</span>
              </label>
            </div>

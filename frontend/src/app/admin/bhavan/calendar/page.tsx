  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-zinc-900">Availability Calendar</h1>
          <p className="text-xs sm:text-sm text-zinc-500 mt-0.5">Effective rule resolution & layer stack inspector per date</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <input
            ...
            className="min-h-[44px] flex-1 sm:flex-none px-3 py-2 border border-zinc-200 rounded-xl text-xs sm:text-sm bg-white"
          />
          ...
          <button
            ...
            className="min-h-[44px] w-full sm:w-auto px-5 py-2.5 bg-amber-500 text-white font-bold rounded-xl text-xs sm:text-sm hover:bg-amber-400 transition-colors shadow-sm"
          >
            Load
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
        ...
        {/* Date Grid */}
        <div className="lg:col-span-2 rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6 shadow-sm">
          {loading ? (
              ...
              {calendarDays.map((day) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 sm:p-3 rounded-xl border cursor-pointer transition-all hover:shadow-md flex flex-col justify-between min-h-[90px] sm:h-24 ${
                      isSelected
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6 shadow-sm space-y-4">
                ...
                {selectedDay.effective_layers.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {selectedDay.effective_layers.map((layer, idx) => (
                      <div key={layer.id} className="p-3 rounded-xl border border-zinc-200 bg-zinc-50 flex items-center justify-between">
                        <div>
                        ...
                        {idx === selectedDay.effective_layers.length - 1 && (
                          <span className="text-[9px] font-bold uppercase bg-amber-500 text-white px-2 py-0.5 rounded-full">WINNER</span>
                        )}
                      </div>
                    ))}
                  </div>

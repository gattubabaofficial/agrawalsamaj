...
export default function AdminScanPage() {
  ...
  return (
    <div className="p-3 sm:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-zinc-900">Event Pass Check-In</h1>
        <p className="text-xs sm:text-sm text-zinc-500 mt-1">Verify tickets and confirm entry for samaj events</p>
      </div>

      {/* Camera Scanning Viewport */}
      {cameraScanning && (
        <div className="bg-white p-4 sm:p-6 rounded-3xl border border-zinc-200 shadow-sm flex flex-col items-center justify-center gap-4 sm:gap-5">
          ...
          <div className="relative w-full max-w-xs sm:max-w-sm aspect-square bg-zinc-950 rounded-2xl overflow-hidden border border-zinc-700 shadow-inner flex items-center justify-center">
            <div id="reader" className="w-full h-full"></div>
            
            {/* Viewfinder Overlay Box */}
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
              <div className="w-48 h-48 sm:w-56 sm:h-56 border-2 border-dashed border-amber-500 rounded-2xl relative shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]">
          ...
          <button
            type="button"
            onClick={stopCamera}
            className="px-6 py-2.5 min-h-[44px] bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-md transition-colors cursor-pointer"
          >
            Cancel Camera Scan
          </button>
        </div>
      ...
      {/* Lookup Form */}
      <form onSubmit={handleLookup} className="bg-white p-4 sm:p-5 rounded-2xl border border-zinc-200 shadow-sm flex flex-col sm:flex-row gap-4 items-end sm:items-center">
        <div className="flex-1 space-y-1.5 w-full">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 min-h-[44px] bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 text-sm font-mono"
            />
          </div>
        </div>
        <div className="flex w-full sm:w-auto gap-2.5 sm:gap-3">
          <button
            ...
            className="flex-1 sm:flex-none px-5 py-2.5 min-h-[44px] bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="flex-1 sm:flex-none px-6 py-2.5 min-h-[44px] bg-zinc-900 hover:bg-zinc-800 text-white font-bold text-sm rounded-xl transition-all shadow-md flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
          >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 animate-scale-up">
          {/* Status and Actions Card */}
          <div className="md:col-span-1 bg-white p-4 sm:p-6 rounded-3xl border border-zinc-200 shadow-sm flex flex-col justify-between gap-6">
            <div className="space-y-4">
              <h3 className="font-bold text-zinc-900 text-sm uppercase tracking-wide">Ticket Status</h3>
              
              <div className="p-4 sm:p-5 rounded-2xl bg-zinc-50 border border-zinc-100 flex flex-col items-center gap-3 text-center">
                {passData.status === "used" ? (
                  ...
                  <>
                    <AlertTriangle className="w-12 h-12 text-amber-500 animate-pulse shrink-0" />
                    <div>
            ...
            {passData.status !== "used" && (
              <button
                ...
                disabled={checkingIn}
                className="w-full py-3 min-h-[44px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm shadow-md transition-colors disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
              >
          ...
          {/* Details Card */}
          <div className="md:col-span-2 bg-white p-4 sm:p-6 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
